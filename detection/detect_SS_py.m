function [ o_bbs, o_bbs_wbg ] = detect_SS_py( i_params, i_img, i_mdl, i_cacheID )
...
for sInd=1:numel(scales)
    ...
    curFeat.img_rsz = padarray(imresize(img, curFeat.scale), [i_mdl.wh_cc(2)-1 i_mdl.wh_cc(1)-1 0]*i_params.feat.HoG.SqCellSize, 'symmetric');
...
% get a score
if i_params.feat.HOG.type < 4
    curFeat = crop3dmat(curImgFeat, [i_luv_cc(2:3)' i_mdl.wh_cc']);
    o_score = i_mdl.w_app'*curFeat(:);
else
    img_rsz = i_featPyr(i_luv_cc(1)).img_rsz;
    img_crop = imcrop(img_rsz, [i_luv_cc(2:3)' i_mdl.wh_cc']*i_params.feat.HoG.SqCellSize);
